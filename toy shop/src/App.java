...
public class App {
    ...
    public static void delivery_toys(Scanner iScanner) throws ToysIsNotCreateException  {
        int n = -1;
        while (n == -1){
            n = inputData("Сколько игрушек завезти: ", iScanner);
        }
        for (int i = 0; i < n; i++) {
    ...
    public static void get_prize() {
        if (prize_list.size() == 0) {
            System.out.println("Нет выйгрышей");
        } else {
            Date date = new Date();
            try (FileWriter fw = new FileWriter("prizes_awarded.txt", StandardCharsets.UTF_8, true)) {
                fw.append(String.format("%s %s\n", prize_list.get(0), date));
                fw.flush();
            } catch (IOException ex) {
                System.out.println(ex.getMessage());
            }
            System.out.printf("Выдан приз %s", prize_list.get(0));
            prize_list.remove(0);
        }
    }
    ...
    public static void head_menu(Scanner iScanner) throws ToysIsNotCreateException {
        ...
        System.out.println("5. Завезти партию игрушек");
